...
Inverting a region/buffer (turning unicode back to LaTeX) does not
work because `robin-invert-region` only works when using single
characters like `?φ` instead of `"φ"` as replacements in the robin
package.  Maybe define a second robin package only for inverting a
region?
